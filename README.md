...
- 프로젝트 문서는 선택한 프로젝트 루트 안의 `docs/`에 두어 backend/frontend/app 같은 하위 단위와 같은 루트에서 추적합니다
...
2. 작업 규모와 위험도를 보고 프로젝트 루트의 `docs/` 아래 task, decision, plan, orchestration 문서가 필요한지 판단합니다.
...
실제로 위임이 발생한 경우 이 역할 분리는 `docs/orchestration/ORCH-<task-id>.md`에 기록되며, `execution_mode`, `supervisor_agent`, `planner_agents`, `reviewer_agents`, `implementation_agents`, `validation_agents`, `owned_scopes`, `risk_preflight_status`, `approval_ref` 같은 필드를 통해 추적됩니다.

서브에이전트나 외부 evaluator를 시작하기 전에는 메인 supervising lead architect가 risk preflight를 먼저 수행해야 합니다. safety, privacy, permission, destructive action, external side effect, sensitive data, scope, user-decision gate가 있으면 delegation/external review를 시작하지 않고, 사용자가 승인한 정확한 다음 단계가 있어야 ping-pong을 시작합니다.
...
- [`scripts/init_docs_scaffold.sh`](./scripts/init_docs_scaffold.sh): 프로젝트 루트 `docs/` 기본 구조와 템플릿 생성
...
- [`scripts/shadow_policy_loader.py`](./scripts/shadow_policy_loader.py): shadow policy Markdown의 fenced `yaml` registry block만 선언 레지스트리로 읽고 probe adapter 구현 목록과 parity를 검사
- [`scripts/shadow_evidence_probe.py`](./scripts/shadow_evidence_probe.py): static `AdapterRegistry`에 등록된 검증기만 실행해 shadow effect 후보를 deterministic probe 결과로 변환
- [`scripts/shadow_review_queue.py`](./scripts/shadow_review_queue.py): 증거가 부족한 effect 후보를 bounded human review queue로 변환
- [`scripts/shadow_llm_candidate_wrapper.py`](./scripts/shadow_llm_candidate_wrapper.py): LLM 초안을 non-promotable candidate artifact로 감쌈
- [`scripts/shadow_user_decision_wrapper.py`](./scripts/shadow_user_decision_wrapper.py): 최종 적용 승인과 human fact-evidence 결정을 guarded user_decision artifact로 생성
- [`scripts/shadow_apply_gate.py`](./scripts/shadow_apply_gate.py): 최종 shadow 적용 승인 조건을 dry-run으로 검사
- [`scripts/shadow_effect_writer.py`](./scripts/shadow_effect_writer.py): gate를 통과한 `confirmed/unknown/blocked/stale` structured effect record를 shadow effect_map에 기록하고, blocked 결과에는 가능한 다음 probe/decision command를 `next_actions`로 제안
...
python3 "$CODEGUIDE_ROOT/scripts/shadow_policy_loader.py" --project-root /path/to/project --adapter-module "$CODEGUIDE_ROOT/scripts/shadow_evidence_probe.py" --check-parity
python3 "$CODEGUIDE_ROOT/scripts/shadow_evidence_probe.py" --project-root /path/to/project --validator-id py.ast.call_match@v1 --source-file src/app.py --callee save
python3 "$CODEGUIDE_ROOT/scripts/shadow_review_queue.py" --input probe.jsonl --task-id shadow-effect-map-01
python3 "$CODEGUIDE_ROOT/scripts/shadow_user_decision_wrapper.py" --project-root /path/to/project --output decision.json --decision-id UD-001 --decision-type final_shadow_apply --answer yes --decided-by gm --decided-at 2026-05-03 --expires-at 2026-05-10 --candidate-id LC-... --record record.json --target-shadow-file packages/app/effects.md --rationale "Approve exact record" --source-ref TASK-shadow-effect-map-01
